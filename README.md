...
- **環境健檢**：偵測柏宝箱在場（自動讓出重疊功能）、雲端用不到的擴充（TauriTavern）、第三方擴充數量過多、「自動載入上次聊天 + 超長聊天」的拖慢組合。只「說」哪裡有問題，不替你改。
...
- **維運提示卡**：把「生成等待久但畫面不忙」這類伺服器側問題**誠實指回伺服器**——memory limit + OOMKill、config 三行。純文字導流，不執行任何伺服器操作。
